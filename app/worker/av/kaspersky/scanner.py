...
from app.worker.av.generic.abstract import AbstractEvilnessEngine, L32_PLATFORM
from app.worker.av.generic.exception import ScannerUpdateError
...
class kaspersky_engine(AbstractEvilnessEngine):
	...
	def version(self):
		...
		version['definitions'] = defns_lines[0].split(':', 1)[1].strip()
		version['last_update'] = defns_lines[1].split(':', 1)[1].strip()
		version['defn_state'] = defns_lines[2].split(':', 1)[1].strip()
